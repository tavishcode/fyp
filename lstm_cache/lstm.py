...
# Play with learning rate
    def reshape(self, data, labels):
        data = np.ravel(data).reshape(
            (self.NUM_CONTENT_TYPES * data.shape[0], self.timesteps, self.features))
        labels = np.ravel(labels).reshape(
            (self.NUM_CONTENT_TYPES * data.shape[0], 1, 1))
    ...
    def train(self, data, labels):
        ...
        checkpoint = ModelCheckpoint(
            "model.hdf5", monitor='loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [checkpoint]
        model = Sequential()
        model.add(LSTM(128, input_shape=(self.timesteps, self.features)))
        # Reshapes output so it is accessible by next layer
