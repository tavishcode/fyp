...
class Graph:
  def __init__(self, cache_size, num_content_types, grid_rows, grid_cols, policy, q, rng):
    ...
    for i in range(0, self.num_routers):
      name = 'r' + str(i)
      self.routers.append(
          Router(cache_size, num_content_types, name, policy, q))
    ...
    for r in self.routers:
      ...
      for p in producers:
        if r == p.gateway:
          ...
          fib[p.content] = self.get_best_hop(
              self.adj_mtx, self.get_pos(r), self.get_pos(p.gateway))
      r.FIB = fib
      ...
      for p in producers:
        ...
        if src.FIB[c] == p:
          if src.contentstore.has(c):
            src.FIB[c] = src
          return
        # reset path to point to gateway
