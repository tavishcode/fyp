...
class GruEncoderDecoder:
  def __init__(self, timesteps, hidden_units, pred_steps):
    ...
    self.decoder = GRU(self.hidden_units, return_state=True,
    ...
    decoder_outputs, decoder_output_h = self.decoder(
        decoder_inputs, initial_state=decoder_input_h)
    decoder_outputs = self.decoder_dense(decoder_outputs)
    self.inference_decoder = Model([decoder_inputs, decoder_input_h], [
                                   decoder_outputs, decoder_output_h])
  ...
  def predict(self, encoder_input):
    ...
    for i in range(self.pred_steps):
      decoder_output, decoder_input_h = self.inference_decoder.predict(
          [target_seq, decoder_input_h])
